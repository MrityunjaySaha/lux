...
const ProductDetails = () => {
  ...
  const [cartProducts, setCartProducts] = useState({}); // Use an object to store quantities for each currency
  ...
  const handleAcceptBarterOffer = async () => {
    try {
      ...
      // Check if serviceDetails exists and has a 'currencies' property
      if (!productDetails || !productDetails.currencies) {
        throw new Error("Service details or currencies are missing.");
      }
      ...
      const updatedBarterQuantity = (productData.currencies.find(currency => currency.currency === 'Barter')?.quantity || 0) - selectedQuantity;

      // Update the service data in the database
      await productRef.update({
        currencies: productData.currencies.map(currency => {
      ...
      console.error("Error while accepting the barter offer:", error.message);
      
      // Display an error message to the user
      toast.error("Error while accepting the barter offer. Please try again.");
    }
  }; 
  ...
  const handlePaymentWithSTar = async () => {
    ...
    try {
        ...
        const starPrice = productDetails?.currencies?.find(currency => currency.currency === 'STar')?.price;
        ...
        const updatedSTarQuantity = productData.currencies.find(currency => currency.currency === 'STar').quantity - selectedQuantity;
  ...
  const barterCurrency = productDetails?.currencies?.find(
  ...
  const totalQuantity = productDetails?.currencies?.reduce(
    (total, currency) => total + currency.quantity,
    0
  );

  const cartProductIncrease = currency => {
    setCartProducts(prevCartProducts => {
      const updatedProducts = { ...prevCartProducts };
      const currentQuantity = updatedProducts[currency] || 0;
      const maxQuantity = productDetails?.currencies?.find(c => c.currency === currency)?.quantity;

      if (currentQuantity < maxQuantity) {
        updatedProducts[currency] = currentQuantity + 1;
  ...
  const cartProductDecrease = currency => {
    setCartProducts(prevCartProducts => {
      if (prevCartProducts[currency] && prevCartProducts[currency] > 0) {
        const updatedProducts = { ...prevCartProducts };
        updatedProducts[currency] = updatedProducts[currency] - 1;
        setSelectedQuantity(prevQuantity => prevQuantity - 1); // Update selectedQuantity
        return updatedProducts;
      }
  ...
  // Modify the calculateTotalPrice function
    const calculateTotalPrice = useCallback((currency, quantity) => {
        const currencyData = productDetails?.currencies?.find(c => c.currency === currency);
        if (currencyData) {
        if (currency !== 'Barter') {
        ...
        return 0;
    }, [productDetails?.currencies, selectedPrice]);

    useEffect(() => {
        const currencyData = productDetails?.currencies?.find(
          currency => currency.currency === 'Sterling'
        );
        if (currencyData) {
          const total = calculateTotalPrice(
            currencyData.currency,
            cartProducts[currencyData.currency] || 0
          );
          setTotalPriceSterling(total);
        }
    }, [selectedPrice, cartProducts, productDetails, calculateTotalPrice]);    
    ...
    const handlePaymentWithSterling = async () => {
        ...
        try {
            ...
            // Retrieve the Sterling price from productDetails or any relevant source
            const sterlingPrice = productDetails?.currencies?.find(currency => currency.currency === 'Sterling')?.price;
            ...
            // Calculate the new Sterling quantity
            const updatedSterlingQuantity = productData.currencies.find(currency => currency.currency === 'Sterling').quantity - selectedQuantity;
    ...
    const handleToken = async (token, selectedQuantity) => {
        ...
        const productLocalCurrency = productDetails?.currencies?.find(currency => currency.currency === 'LocalCurrency')?.selectedOptions[0].value;
        if (productLocalCurrency !== selectedLocalCurrency) {
        ...
        try {
          const response = await axios.post('http://localhost:8080/product-checkout', requestData);
          ...
          if (status === 'success') {
            ...
            const updatedQuantity = productData.currencies.find(currency => currency.currency === 'LocalCurrency').quantity - selectedQuantity;
    ...
    <div>
      ...
      <div className="product">
            ...
            <div>
              ...
              {productDetails?.currencies?.map((currencyData) => (
            ...
            <div>
                <strong>Uploader:</strong> {productDetails?.username}
            </div>
            <div>
                <strong>Display Name:</strong> {productDetails?.displayName}
            </div>
            <div>
                <strong>Address:</strong> {productDetails?.address}
            </div>
            <div>
                <strong>City:</strong> {productDetails?.city} {/* Display city */}
            </div>
            <div>
                <strong>State/Union Territory:</strong> {productDetails?.stateOrTerritory} {/* Display state/union territory */}
            </div>
            <div>
                <strong>Zip Code:</strong> {productDetails?.zipCode} {/* Display zip code */}
            </div>
            <div>
                <strong>Country:</strong> {productDetails?.country}
            </div>
            <div>
                <strong>Phone Number:</strong> {productDetails?.phoneNumber}
            </div>
            <div>
                <strong>Email:</strong> {productDetails?.email}
            </div>
            <div>
                <strong>Uploaded:</strong> {productDetails?.timestamp}
            </div>
        </div>
            ...
            {currencyData.currency === 'LocalCurrency' ? (
                ...
                <div className="quantity-control">
                    ...
                    <div className="quantity-box">
                        <div className="action-btns minus" onClick={() => cartProductDecrease(currencyData.currency)}>
                            <Icon icon={minus} size={20} />
                        </div>
                        <div className="quantity">{cartProducts[currencyData.currency] || 0}</div>
                        <div className="action-btns plus" onClick={() => cartProductIncrease(currencyData.currency)}>
                        ...
                        {currencyData.currency === 'Barter' && (
                            <div>
                                Quantity: {cartProducts[currencyData.currency] || 0}
                            </div>
                        )}
                        ...
                        {currencyData.currency !== 'Barter' && (
                            <div>
                                Quantity: {cartProducts[currencyData.currency] || 0}<br></br>
                                Price: {currencyData.currency === 'Sterling' ? (
                                    ...
                                    <span>
                                        {currencyData.price} <br></br>
                                        Total Price: {calculateTotalPrice(currencyData.currency, cartProducts[currencyData.currency] || 0)}<br></br>
                                ...
                                {console.log('Total Price:', calculateTotalPrice(currencyData.currency, cartProducts[currencyData.currency] || 0))}
                            </div>
                        )}
                        <br></br>
